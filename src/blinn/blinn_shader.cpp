...
glm::vec4 BlinnShader::ExecuteVertexShader(void* vs_in, void* fs_in, void* uniforms) {
    ...
    auto world_position = glm::vec3(world_position);
    auto depth_position = glm::vec3(light_vp_mat*world_position);

    if (_uniforms->shadow_pass) {
        _fs_in->texcoord = texcoord;
        return glm::vec4(depth_position, 1);
    } 
    else {
        _fs_in->normal = glm::normalize(normal_mat*normal);
        _fs_in->world_position = world_position;
        _fs_in->depth_position = depth_position;
        _fs_in->texcoord = texcoord;
